...
export default {
  ...
  methods: {
    getList() {
      ...
      const { res } = fetchAdminList(this.searchQueryCopy)
      if (res || 1) {
    ...
    handleModifyStatus(row, status) {
      ...
      const { res } = activateOrFreezeAdmin(obj)
      if (res || 1) {
        this.$message({
          message: '操作Success',
          type: 'success'
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const { res, err } = createAdmin(this.temp)
          if (res || 1) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const { res, err } = updateAdmin(tempData)
          if (res || 1) {
    ...
    handleDelete(row, index) {
      const { res } = deleteAdmin(row.user_id)
      if (res || 1) {
        this.$notify({
          title: 'Success',
